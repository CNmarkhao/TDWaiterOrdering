package com.tdlbs.waiterordering.crash;
...
public class LogToFileUtils {
     ...
     * 默认10M
     */
    private static final int LOG_MAX_SIZE = 300 * 1024 * 1024;
     ...
     * @param str 需要写入的数据
     */
    public static void crash(Object str) {
        // 判断是否初始化或者初始化是否成功
        logFile = getCrashFile();
        if (null == mContext || null == instance || null == logFile || !logFile.exists()) {
            Log.e(MY_TAG, "Initialization failure !!!");
            return;
        }
    ...
    private static File getLogFile() {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            file = new File(Environment.getExternalStorageDirectory().getPath() + AppConstants.FilePath.APP_LOG_PATH);
        } else {
            // 没有SD卡或者外部存储器，使用内部存储器
            // \data\data\包名\files\Log\logs.txt
            file = new File(mContext.getFilesDir().getPath() + AppConstants.FilePath.APP_LOG_PATH);
        }
        // 若目录不存在则创建目录
        if (!file.exists()) {
            file.mkdirs();
        }
        String time = TimeUtils.getNowString(fileSDF);
        if (time.length() != 8) {
            return null;
        }
        File logFile = new File(file.getPath() + "/" + time + ".txt");
        if (!logFile.exists()) {
            try {
                logFile.createNewFile();
            } catch (Exception e) {
    ...
    private static File getCrashFile() {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            file = new File(Environment.getExternalStorageDirectory().getPath() + AppConstants.FilePath.APP_CRASH_PATH);
        } else {
            // 没有SD卡或者外部存储器，使用内部存储器
            // \data\data\包名\files\Log\logs.txt
            file = new File(mContext.getFilesDir().getPath() + AppConstants.FilePath.APP_CRASH_PATH);
        }
        // 若目录不存在则创建目录
        if (!file.exists()) {
            file.mkdirs();
        }
        File logFile = new File(file.getPath() + "/" + fileSDF.format(new Date()) + "-crash.txt");
        if (!logFile.exists()) {
            try {
                logFile.createNewFile();
            } catch (Exception e) {
    ...
    private static String getFunctionInfo() {
        ...
        for (StackTraceElement st : sts) {
            ...
            tag = st.getFileName();
            return "[" + logSDF.format(new Date()) + " v" + BuildConfig.VERSION_NAME + " " + st.getClassName() + " " + st
